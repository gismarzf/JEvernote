...
public class EDAMDemo {
	...
	private static final String AUTH_TOKEN =
			"S=s1:U=8b9d6:E=149a4d264f3:C=1424d2138f4:P=1cd:A=en-devtoken:V=2:H=b91f9552fe434cef234885ebe369586b";
	...
	public static void main(String args[]) throws Exception {
		...
		try {
			...
			if (e.getErrorCode() == EDAMErrorCode.AUTH_EXPIRED) {
				System.err
						.println("Your authentication token is expired!");
			} else if (e.getErrorCode() == EDAMErrorCode.INVALID_AUTH) {
				System.err
						.println("Your authentication token is invalid!");
	...
	public EDAMDemo(String token) throws Exception {
		...
		EvernoteAuth evernoteAuth =
				new EvernoteAuth(EvernoteService.SANDBOX, token);
	...
	private void createNote() throws Exception {
		...
		System.out
				.println("Successfully created a new note with GUID: "
						+ newNoteGuid);
		System.out.println();
	}
	 ...
	 * Data object.
	 */
	private static Data readFileAsData(String fileName)
			throws Exception {

		// Read the full binary contents of the file
		FileInputStream in =
				new FileInputStream(EDAMDemo.class.getResource(
						"enlogo.png").getFile());
